...
public class PasswordResolverFactory {
     ...
     */
    public static void initializePasswordResolver()
            throws APIManagementException {

        APIManagerConfiguration configuration = ServiceReferenceHolder.getInstance()
                .getAPIManagerConfigurationService().getAPIManagerConfiguration();
        if (configuration != null) {
            String passwordResolverImplClass = configuration.getFirstProperty(APIConstants.PASSWORD_RESOLVER_IMPL_CLASS);
            if (passwordResolverImplClass == null) {
                passwordResolver = new DefaultPasswordResolverImpl();
            } else {
                try {
                    passwordResolver = (PasswordResolver) APIUtil.getClassForName(passwordResolverImplClass).newInstance();
